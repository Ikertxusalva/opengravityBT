...
=====================================
Sistema completo de trading en mercados de prediccion.

PAPER_MODE = True  ->  Simula ordenes con datos reales. NO mueve dinero.
PAPER_MODE = False ->  Ejecuta ordenes reales en Polygon via CLOB.
                       (Requiere credenciales configuradas)

Uso:
    python paper_trader.py audit         # Auditar posiciones: cierra las de edge invalido
    python paper_trader.py audit-dry     # Audit sin cerrar (solo diagnostico)
    ...
    python paper_trader.py report        # Reporte completo con metricas
    python paper_trader.py update        # Actualizar precios + stop-loss
    python paper_trader.py loop [min]    # Ciclo automatico (default: 30 min)
"""
...
try:
    ...
    from signals.llm_estimator import is_available as llm_is_available
    LLM_AVAILABLE = True
except ImportError:
    LLM_AVAILABLE = False

    def llm_estimate(*args, **kwargs):
        return None

    def llm_is_available():
        return False

# News fetcher (GDELT + RSS)
try:
    from signals.news_fetcher import fetch_news_for_market
    NEWS_AVAILABLE = True
except ImportError:
    NEWS_AVAILABLE = False

    def fetch_news_for_market(*args, **kwargs):
        return ""

# Supabase integration
try:
    from db.supabase_client import (
        save_position as supa_save_position,
        get_open_positions as supa_get_open_positions,
        close_position as supa_close_position,
        save_signal_score as supa_save_signal_score,
        is_available as supa_is_available,
    )
    SUPABASE_MODULE = True
except ImportError:
    SUPABASE_MODULE = False

    def supa_save_position(*args, **kwargs):
        return False

    def supa_get_open_positions():
        return None

    def supa_close_position(*args, **kwargs):
        return False

    def supa_save_signal_score(*args, **kwargs):
        return False

    def supa_is_available():
        return False
...
#  --- UNICO PUNTO DE CONTROL PAPER vs REAL ---
# ==============================================================================
...
# Parametros de riesgo (aplican en PAPER y en REAL)
MAX_POSITION    = 300      # USD maximo por posicion
MAX_EXPOSURE    = 2_000    # USD maximo total desplegado
MIN_EDGE        = 0.05     # Edge minimo para entrar (5 puntos)
MIN_LIQUIDITY   = 2_000    # Liquidez minima en order book
MIN_VOLUME      = 30_000   # Volumen total minimo del mercado
MIN_DAYS_LEFT   = 3        # Dias minimos hasta resolucion
MAX_SPREAD      = 0.15     # Spread maximo tolerado
STOP_LOSS_PCT   = -0.30    # Cerrar si perdida > 30% del capital invertido
...
MIN_SIGNAL_SOURCES = 2     # PHASE 1 FIX: Minimo 2 fuentes independientes para abrir

# Audit thresholds — detectan posiciones con edge invalido
AUDIT_LONG_HORIZON_DAYS = 90   # Horizonte largo con fuente única = capital bloqueado
AUDIT_PHANTOM_EDGE_THRESHOLD = 0.03  # Edge re-evaluado < 3pts = fantasma

# APIs Polymarket (publicas, sin auth para lectura)
...
# ==============================================================================
#  BAYESIAN KELLY -- Tamano de posicion con incertidumbre
# ==============================================================================

class BayesianKelly:
    """
    Calcula el Kelly optimo usando una distribucion Beta sobre la
    probabilidad de ganar. Se actualiza con cada trade resuelto.

    P(win) ~ Beta(alpha, beta)
    E[p]   = alpha / (alpha + beta)
    Var[p] = alpha*beta / (alpha+beta)^2*(alpha+beta+1)

    Kelly robusto = E[p] - confidence_factor * StdDev[p]
    -> A menor certeza, posicion mas pequena.
    """
    ...
    def kelly_fraction(self, source: str, entry_price: float,
        ...
        if n_samples < 5:
            # Bootstrap: Kelly directo con p_win, sin penalizacion bayesiana
...
def get_clob_token_ids(condition_id: str) -> tuple[Optional[str], Optional[str]]:
    """Obtiene yes_token_id y no_token_id del CLOB (formato correcto de 77 digitos)."""
...
def get_clob_market_data(condition_id: str) -> Optional[dict]:
    ...
    Reemplaza get_clob_token_ids() + get_orderbook() para la mayoria de casos.
    """
    try:
        ...
        if yes_price <= 0 or no_price <= 0:
            return None
        # Spread estimado desde la suma (idealmente suman 1.0; desviacion = spread)
        price_sum = yes_price + no_price
        spread_est = abs(1.0 - price_sum) + 0.02  # minimo 2% spread
        return {
...
# ==============================================================================
#  DETECCION DE EDGE -- Senales de mispricing
# ==============================================================================

# PHASE 1 FIX: Calibrated base rates with realistic probabilities.
# Previous version had "putin" at 0.50 which is unrealistic.
# Key changes:
# - Sitting heads of state: 85-95% (they rarely lose power unexpectedly)
# - Geopolitical events like ceasefire/peace: lowered (historically rare)
# - Political leaders staying in power: high base rates
# - Rare events (impeachment, arrest): very low base rates
BASE_RATES = {
    # -- Crypto ----------------------------------------------------------------
    ...
    # -- Politica EE.UU. -------------------------------------------------------
    # Sitting presidents/leaders have high base rates of staying in power
    "trump": 0.55,           "harris": 0.40,    "biden": 0.35,
    "re-elected": 0.55,      "reelected": 0.55, "incumbent": 0.58,
    "senate": 0.50,          "house": 0.50,     "midterm": 0.50,
    "presidential election": 0.50,
    "impeach": 0.08,         "resign": 0.07,    "pardon": 0.45,
    "convicted": 0.25,       "arrested": 0.10,  "indicted": 0.30,
    "tariff": 0.55,          "sanction": 0.50,  "executive order": 0.55,

    # -- Politica Mundial / Geopolitica ----------------------------------------
    # PHASE 1 FIX: Leaders in power stay in power ~85-95% of the time.
    # "putin" was 0.50 which is absurd; he's been in power since 1999.
    # Questions about Putin typically ask "will Putin still be president" -> ~93%
    # or "will Putin do X aggressive action" -> depends on context, use ~0.40 for actions
    "putin": 0.40,           "putin president": 0.93,  "putin power": 0.93,
    "xi jinping": 0.93,      "xi president": 0.95,
    "erdogan": 0.88,         "modi": 0.85,
    "zelensky": 0.50,        "zelensky president": 0.75,
    "netanyahu": 0.70,       "netanyahu prime minister": 0.75,
    "macron": 0.85,          "starmer": 0.80,
    "election": 0.50,        "prime minister": 0.50,
    "ceasefire": 0.25,       "peace deal": 0.15, "peace agreement": 0.15,
    "war": 0.40,             "invasion": 0.20,   "nato": 0.50,
    "coup": 0.05,            "assassination": 0.03,
    "regime change": 0.08,   "overthrow": 0.05,
    ...
    # -- Finanzas / Empresas ---------------------------------------------------
    "bankrupt": 0.10,        "default": 0.12,   "collapse": 0.10,
    "layoff": 0.50,          "earnings": 0.50,

    # -- Deportes: Clasificacion / Playoff (50/50 inherente) -------------------
    ...
    # -- Deportes: Torneo (1/N; usamos avg historico ~10% para equipos no fav) -
    ...
    "win serie a": 0.12,          "win ligue 1": 0.12,
    "ballon d'or": 0.10,          "mvp": 0.10,
    ...
    # -- Geopolitico / Desastres -----------------------------------------------
    "earthquake": 0.20,      "hurricane": 0.30,  "pandemic": 0.10,
    "nuclear": 0.03,         "world war": 0.02,
}
...
def check_base_rate(question: str, price_yes: float) -> Optional[dict]:
    ...
    best_match = None
    best_edge = 0.0
    # PHASE 1 FIX: Use longest keyword match first (more specific = better)
    # Sort by keyword length descending so "putin president" matches before "putin"
    sorted_rates = sorted(BASE_RATES.items(), key=lambda x: len(x[0]), reverse=True)
    for kw, rate in sorted_rates:
        if kw in q:
            edge = rate - price_yes
...
def check_overreaction(condition_id: str, lookback_h=24) -> Optional[dict]:
    ...
    # Asumimos reversion parcial del 40%
...
def check_wide_spread(ob: dict) -> Optional[dict]:
    ...
    ofrecen edge al tomar posicion en la direccion del mid.
    """
    spread = ob["spread"]
    mid = ob["mid"]
...
def check_yes_no_sum(clob_data: dict) -> Optional[dict]:
    """
    Detecta cuando YES_price + NO_price se desvia de 1.0.
    - sum < 0.90: ambos tokens subvalorados (edge comprando el mas alejado del fair value)
    - sum > 1.10: vigorish excesivo (evitar, o BUY_NO si YES esta inflado)
    """
    ...
    if price_sum < 0.90:
        ...
        if edge < MIN_EDGE:
            return None
        # Comprar el que tiene mas descuento respecto a fair value (0.5 base)
    ...
    # Sum alto: market maker cobrando mucho vig o yes esta inflado
    if price_sum > 1.10:
        gap = price_sum - 1.0
        edge = gap * 0.5
        if edge < MIN_EDGE:
            return None
        # El que tiene precio mas alto esta mas overpriced -> BUY_NO de ese
...
def check_volume_spike(m: dict, price_yes: float) -> Optional[dict]:
    ...
    Indica actividad informada reciente -> precio podria moverse mas.
    Edge: si ya se movio, hay momentum; si no, hay informacion pendiente de pricear.
    """
    ...
    # Dias de vida estimados desde volumen total (minimo 7 para evitar mercados nuevos)
    ...
    # Spike alto + precio en zona de incertidumbre = senal de movimiento
    ...
    # Direccion: si precio < 0.5, el spike sugiere BUY_YES (mas compradores)
...
def check_tournament_relative_value(m: dict, markets_pool: list,
    ...
    """
    Detecta anomalias de precio relativo en grupos de mercados del mismo torneo.
    ...
    # Extraer "grupo" por las ultimas 3-4 palabras significativas del torneo
    group_keywords = []
    ...
    # Solo senal si la suma total es significativa (grupo bien representado)
    if price_sum < 0.5:
        return None
    ...
    # Senal si el precio esta muy alejado del fair share ajustado
    threshold = max(0.08, adjusted_fair * 0.5)  # 50% de desviacion o minimo 8pts
...
def analyze_market(m: dict, markets_pool: Optional[list] = None, cross_signals: Optional[dict] = None) -> Optional[dict]:
    ...
    # -- PASO 1: Filtros rapidos con datos de Gamma (sin calls al CLOB) --------
    ...
    # Pre-filtro de precio con outcomePrices de Gamma (muy rapido)
    ...
    # Construir ob basico desde datos CLOB + Gamma
    if best_bid_g > 0 and best_ask_g > 0:
        ob = {
        ...
        # Usar precio CLOB si esta disponible (mas actualizado)
    ...
    # -- PASO 3: Deteccion de edge ---------------------------------------------
    checks = []

    # 1. Sum arbitrage YES+NO (usa datos CLOB directos)
    ...
    # 5. Tournament relative value (solo si hay pool)
    if markets_pool:
    ...
    # -- Senales externas (solo si SIGNALS_AVAILABLE) --------------------------
    if SIGNALS_AVAILABLE and checks:
        ...
        # 8. Manifold signal
        # PHASE 1 FIX: Raised min_similarity from 0.28 to 0.40 to eliminate
        # false positive matches between unrelated markets
        try:
            mf = check_manifold_signal(m["question"], price_yes,
                                       min_edge=0.08, min_similarity=0.40)
            if mf:
                checks.append(mf)
        except Exception:
            pass
        ...
        # PHASE 1 FIX: Use CLOB token IDs (from get_clob_market_data) not Gamma IDs
        # The yes_token variable already holds the correct CLOB token ID
    ...
    # -- PHASE 3/5: LLM probability estimator (Groq) --------------------------
    # Only call LLM for markets that already have at least 1 signal
    # to avoid wasting API calls on markets with no edge
    if LLM_AVAILABLE and llm_is_available() and checks:
        try:
            # Fetch news context for this market
            news_ctx = ""
            if NEWS_AVAILABLE:
                news_ctx = fetch_news_for_market(m["question"], max_articles=5)

            llm_result = llm_estimate(m["question"], price_yes, news_context=news_ctx)
            if llm_result and abs(llm_result.get("edge", 0)) >= MIN_EDGE:
                checks.append({
                    "source": "llm",
                    "edge": round(abs(llm_result["edge"]), 4),
                    "direction": llm_result["direction"],
                    "estimated_prob": llm_result.get("estimated_prob"),
    ...
    # -- PHASE 1 FIX: Require at least MIN_SIGNAL_SOURCES (2) independent -----
    # sources before opening a position. This prevents base_rate alone from
    # triggering trades (which was the main source of false positives).
    unique_sources = set(c["source"] for c in checks)
    if len(unique_sources) < MIN_SIGNAL_SOURCES:
        return None

    # -- Consenso de direccion -------------------------------------------------
    dirs = [c["direction"] for c in checks]
    direction = max(set(dirs), key=dirs.count)

    if dirs.count(direction) < len(dirs) * 0.6:
        return None  # senales contradictorias
    ...
    # -- Log signal scores to Supabase -----------------------------------------
    if SUPABASE_MODULE and supa_is_available():
        score_dict = {}
        for c in checks:
            src = c["source"]
            if src == "whale_clob":
                score_dict["whale_score"] = c["edge"]
            elif src == "ob_imbalance":
                score_dict["orderbook_score"] = c["edge"]
            elif src == "manifold_signal":
                score_dict["manifold_score"] = c["edge"]
            elif src == "kalshi_arb":
                score_dict["kalshi_score"] = c["edge"]
            elif src in ("cross_market_ladder", "tournament_rv"):
                score_dict["cross_market_score"] = c["edge"]
            elif src == "llm":
                score_dict["llm_score"] = c["edge"]
            elif src == "base_rate":
                score_dict["base_rate_score"] = c["edge"]
        score_dict["composite_edge"] = composite_edge
        score_dict["direction"] = direction
        try:
            supa_save_signal_score(condition_id, m.get("question", ""), score_dict)
        except Exception:
            pass
...
# ==============================================================================
#  MOTOR DE ORDENES -- Paper mode vs Real mode
# ==============================================================================

def execute_order(signal: dict, size_usd: float, db: dict) -> bool:
    """
    Punto central de ejecucion. En PAPER_MODE simula la orden.
...
def _paper_fill(signal: dict, size_usd: float, db: dict) -> bool:
    """Simula un fill: registra posicion virtual."""
    ...
    if SUPABASE_MODULE:
        try:
            supa_save_position(position)
        except Exception:
            pass

    print(f"""
  [OK] POSICION ABIERTA [PAPER]
     ID:         {position['id']}
     Mercado:    {position['question'][:70]}
     Dir:        {position['direction']} @ ${entry:.4f}
     Tamano:     ${size_usd:.2f}  ({shares:.2f} shares)
     Edge:       {position['composite_edge']:.2%}
     Sources:    {', '.join(position['edge_sources'])}
     Stop-loss:  ${position['stop_loss_price']:.4f}
     Take-profit:${position['take_profit_price']:.4f}
     Vence:      {position['end_date']} ({position['days_left']} dias)""")
    return True
...
def _real_fill(signal: dict, size_usd: float, db: dict) -> bool:
    ...
    try:
        ...
        print(f"  [!] Error en ejecucion real: {e}")
        return False


# ==============================================================================
#  GESTION DE POSICIONES -- Stop-loss, Take-profit, Resolve
# ==============================================================================

def _close_position(pos: dict, final_price: float, reason: str,
                    db: dict, bk: BayesianKelly):
    """Cierra una posicion y actualiza stats + priors bayesianos."""
    ...
    if SUPABASE_MODULE:
        try:
            supa_close_position(pos["condition_id"], final_price, reason, pos)
        except Exception:
            pass
...
# ==============================================================================
#  AUDIT DE PORTFOLIO -- Detecta edge fantasma y cierra posiciones invalidas
# ==============================================================================

def _audit_position(pos: dict) -> dict:
    """
    Re-evalua la calidad del edge de una posicion abierta.
    Devuelve un dict con risk_level, should_close, issues.

    Criterios de cierre automatico (CRITICAL):
    1. Unica fuente de edge Y horizonte > AUDIT_LONG_HORIZON_DAYS dias
    2. Base rate re-evaluado contradice la direccion de la posicion
    3. Edge re-evaluado < AUDIT_PHANTOM_EDGE_THRESHOLD con fuente unica

    Criterios de alerta (HIGH) sin cierre automatico:
    4. Unica fuente de edge Y horizonte corto (<= 90 dias)
    """
    issues = []
    risk_level = "LOW"
    should_close = False
    ...
    sources = pos.get("edge_sources", [])
    unique_sources = set(sources)
    n_sources = len(unique_sources)
    days_left = pos.get("days_left", 999)
    entry_price = pos.get("entry_price", 0.5)
    current_price = pos.get("current_price", entry_price)
    direction = pos.get("direction", "BUY_YES")

    # --- 1. Fuente unica ---------------------------------------------------------
    if n_sources < MIN_SIGNAL_SOURCES:
        issues.append(
            f"Solo {n_sources} fuente(s) de edge — abierta antes de requerir {MIN_SIGNAL_SOURCES}"
        )
        risk_level = "HIGH"

        # --- 2. Horizonte largo con fuente unica = capital muerto ----------------
        if days_left > AUDIT_LONG_HORIZON_DAYS:
            issues.append(
                f"{days_left} dias restantes con fuente unica — capital bloqueado sin confirmacion"
            )
            risk_level = "CRITICAL"
            should_close = True
            close_reason = "single_source_long_horizon"

        # --- 3. Re-evaluar base rate con tasas corregidas ------------------------
        if "base_rate" in unique_sources:
            recheck = check_base_rate(pos["question"], current_price)
            if recheck:
                new_edge = recheck["edge"]  # positivo = BUY_YES, negativo = BUY_NO
                new_dir = recheck["direction"]
                if new_dir != direction and abs(new_edge) > AUDIT_PHANTOM_EDGE_THRESHOLD:
                    issues.append(
                        f"Base rate corregido dice {new_dir} (edge={new_edge:+.2%}) — "
                        f"contradice posicion {direction}"
                    )
                    risk_level = "CRITICAL"
                    should_close = True
                    close_reason = "base_rate_contradicts_direction"
                elif abs(new_edge) < AUDIT_PHANTOM_EDGE_THRESHOLD and n_sources < 2:
                    issues.append(
                        f"Edge re-evaluado marginal ({new_edge:+.2%}) con fuente unica — "
                        f"edge fantasma"
                    )
                    risk_level = "CRITICAL"
                    should_close = True
                    close_reason = "phantom_edge"
            else:
                # Keyword de base rate ya no matchea con tasas corregidas
                if n_sources == 1:
                    issues.append(
                        "Base rate keyword no encontrada en tasas calibradas — "
                        "edge era especulativo"
                    )
                    risk_level = "CRITICAL"
                    should_close = True
                    close_reason = "base_rate_keyword_missing"

    # --- 4. Precio cerca del stop-loss -------------------------------------------
    pct = (current_price - entry_price) / entry_price if entry_price > 0 else 0
    if pct <= STOP_LOSS_PCT * 0.75 and risk_level == "LOW":
        issues.append(
            f"Precio en {pct:+.1%} — cerca del stop-loss ({STOP_LOSS_PCT:.0%})"
        )
        risk_level = "MEDIUM"
...
def cmd_audit(auto_close=True):
    """
    Audita todas las posiciones abiertas contra los criterios actuales.
    Detecta edge fantasma y cierra posiciones invalidas automaticamente.
    """
    db = load_db()
    bk = load_priors()

    print(f"\n{'='*65}")
    print(f"  AUDIT DE PORTFOLIO  [min {MIN_SIGNAL_SOURCES} fuentes | horizonte max {AUDIT_LONG_HORIZON_DAYS}d]")
    ...
    # Actualizar precios antes de auditar
    print(f"  Actualizando precios de {len(open_positions)} posiciones...")
    ...
    # Auditar cada posicion
    audits = [(pos, _audit_position(pos)) for pos in open_positions]

    # Ordenar: CRITICAL primero
    risk_order = {"CRITICAL": 0, "HIGH": 1, "MEDIUM": 2, "LOW": 3}
    audits.sort(key=lambda x: risk_order.get(x[1]["risk_level"], 4))
    ...
    print(f"\n  {len(audits)} posiciones auditadas:\n")
    for pos, audit in audits:
        icon = risk_icons.get(audit["risk_level"], "[?]")
        pct = audit["pct_change"]
        print(f"  {icon} {audit['question']}")
        print(f"       Fuentes: {', '.join(sorted(audit['sources']))}  |  "
              f"Precio: {audit['entry_price']:.4f} -> {audit['current_price']:.4f} "
              f"({pct:+.1f}%)  |  Dias: {audit['days_left']}")
        for issue in audit["issues"]:
            print(f"       *** {issue}")
        if audit["should_close"]:
            print(f"       --> CERRAR ({audit['close_reason']})")
        print()

    # --- Auto-cierre de posiciones CRITICAL ------------------------------------
    to_close = [(pos, audit) for pos, audit in audits if audit["should_close"]]

    if to_close and auto_close:
        print(f"  Cerrando {len(to_close)} posicion(es) con edge invalido...")
        print(f"  {'-'*63}")
        closed_ids = []
        for pos, audit in to_close:
            exit_price = audit["current_price"]
            _close_position(pos, exit_price, audit["close_reason"], db, bk)
            closed_ids.append(pos["id"])

        db["positions"] = [p for p in db["positions"] if p["id"] not in closed_ids]
        save_db(db)
        print(f"\n  {len(to_close)} cerrada(s). Banco: ${db['bank']:.2f} | "
              f"Desplegado: ${db['deployed']:.2f}")

    elif not to_close:
        save_db(db)  # Guardar precios actualizados
        print("  Todas las posiciones pasan el audit.")
    ...
    still_open = [p for p in db["positions"] if p["status"] == "open"]
    if still_open:
        high_risk = [a for _, a in audits
                     if a["risk_level"] in ("HIGH", "MEDIUM") and not a["should_close"]]
        if high_risk:
            print(f"\n  [!] {len(high_risk)} posicion(es) con riesgo ALTO/MEDIO mantenidas.")
            print(f"      Considera cerrarlas manualmente o monitorear de cerca.")
...
def cmd_resolve():
    """Resuelve posiciones donde el mercado ya cerro."""
    db = load_db()
    bk = load_priors()
    ...
    for pos in db["positions"]:
        ...
        if not (is_closed or is_resolved):
            # Verificar si la end_date ya paso
...
def cmd_scan(auto_open=True):
    ...
    integrations = []
    if LLM_AVAILABLE and llm_is_available():
        integrations.append("LLM(Groq)")
    if NEWS_AVAILABLE:
        integrations.append("News(GDELT)")
    if SUPABASE_MODULE and supa_is_available():
        integrations.append("DB(Supabase)")
    if SIGNALS_AVAILABLE:
        integrations.append("Signals(Kalshi+Manifold+Whale)")
    if integrations:
        print(f"  Integraciones: {' | '.join(integrations)}")
    print(f"  Min fuentes requeridas: {MIN_SIGNAL_SOURCES}")
    print(f"{'='*65}\n")
    ...
    print(f"\n\n  {'-'*63}")
    print(f"  SENALES CON EDGE >= {MIN_EDGE:.0%} (min {MIN_SIGNAL_SOURCES} fuentes): {len(signals)}")
    print(f"  {'-'*63}")
    ...
    for i, s in enumerate(signals, 1):
        ...
        print(f"""
  [{i}] {s['question'][:72]}
       YES={s['price_yes']:.2%}  NO={s['price_no']:.2%}  spread={s['spread']:.4f}  dias={s['days_left']}
    ...
    if auto_open:
        ...
        for s in signals[:5]:
            ...
            # Tamano de posicion con Bayesian Kelly x confluence
            primary = s["checks"][0]["source"]
            kelly_f = bk.kelly_fraction(primary, s["entry_price"], edge=s["composite_edge"])
            pos_usd = min(kelly_f * STARTING_BANK, MAX_POSITION)
...
def cmd_report():
    ...
    if stats["trades"] >= 10:
        ...
        if wr >= 60 and stats["total_pnl"] > 0:
            ...
            print("  [!]  Edge marginal. Necesitas mas datos (>=20 trades).")
        else:
            print("  [X] Edge insuficiente. Revisa las senales.")
    else:
        needed = 10 - stats["trades"]
        print(f"  [i] Necesitas {needed} trade(s) mas para diagnostico confiable.")
...
def cmd_loop(interval_min=30):
    ...
    try:
        ...
        while True:
            ...
            print(f"{'-'*65}")
            cmd_audit(auto_close=True)   # Primero auditar y cerrar posiciones invalidas
            cmd_scan(auto_open=True)
            cmd_update()
            cmd_resolve()
            print(f"\n  Proximo ciclo en {interval_min} min...")
...
def cmd_cycle():
    """Single cycle: audit -> update prices -> resolve closed -> scan for new edge. Returns JSON summary."""
    import json as _json

    # 0. Audit: close invalid positions before anything else
    cmd_audit(auto_close=True)
